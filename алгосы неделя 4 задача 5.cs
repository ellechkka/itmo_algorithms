            ...
            using (var outputFile = new StreamWriter("output.txt")) {
                ...
                while (i < commands.Count) {
                    ...
                    switch (command[0]) {
                        ...
                        case 'P':
                            if (command.Length == 1) {
                                outputFile.WriteLine(queue.Dequeue().ToString());
                            }
                            else {
                                outputFile.WriteLine(registers[command[1]]);
                            }
                            i++;
                            break;
                        case 'C':
                            if (command.Length == 1) {
                                outputFile.Write((char) (queue.Dequeue() % 256));
                            }
                            else {
                                outputFile.Write((char) (registers[command[1]] % 256));
                            }
                            i++;
                            break;
                        case 'J':
